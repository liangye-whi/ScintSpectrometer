...
namespace
{
  void PrintUsage()
  {
    ...
    G4cerr << " OpNovice [-g gdmlfile] [-m macro ] [-u UIsession] [-t "
              "nThreads] [-r seed] [-o outfile]"
           << G4endl;
#else
    G4cerr << " OpNovice  [-m macro ] [-u UIsession] [-t nThreads] [-r seed] [-o outfile] "
           << G4endl;
#endif
...
int main(int argc, char** argv)
{
   ...
   * OPEN ROOT FILE
   */
  HistoManager* fHistoManager = new HistoManager(); 
  fHistoManager->Book(outfile);

  runManager->SetUserInitialization(new OpNoviceActionInitialization(fHistoManager));
